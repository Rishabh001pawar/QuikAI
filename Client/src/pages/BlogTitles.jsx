...
const BlogTitles = () => {
  ...
  const blogCategories=['General', 'Technology', 'Health', 'Lifestyle', 'Education', 'Travel', 'Food', 'Business', 'Science']
    ...
    const onSubmitHandler = async (e) => {
      ...
      try {
        ...
        const { data } = await axios.post('/api/ai/genrate-blog-title', { prompt }, {
          headers: {
    ...
    <div className='flex flex-col md:flex-row gap-6'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full md:w-1/2 p-4 bg-white rounded-lg border border-gray-200'>
        <div className='flex items-center gap-3'>
          ...
          <h1 className='text-lg font-semibold'>AI Blog Title Generator</h1>
      ...
      <div className='w-full md:w-1/2 p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96'>
        <div className='flex items-center gap-3 mb-4'>
          ...
          <h1 className='text-xl font-semibold'>Generated Blog Title</h1>
        </div>

        {
          ...
          <div className='text-sm flex flex-col items-center gap-3 text-gray-400'>
            <Hash className='w-6 h-9' />
            <p>Enter a keyword to generate a blog title</p>
          </div>
        </div>
          ) : (
